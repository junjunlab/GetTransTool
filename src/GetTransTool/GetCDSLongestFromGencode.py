def main():
        ...
        for line in pc:
            if line.startswith('>'):
                ...
                if fileds[8].startswith('CDS'):
                    ...
                    fullname = '_'.join([gene_name,gene_id,trans_id,cds_start,cds_end,trans_length])
                    # save
    ...
    tmp = [[key,key.split(sep='_')[0],int(key.split(sep='_')[5]),int(val)] for key,val in cdsinfo.items()]
